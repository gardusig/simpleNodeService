...
describe("[e2e] Authentication Middleware", () => {
  ...
  describe("[header] Empty Header", () => {
    it("should return HTTP 401 - UNAUTHORIZED", async () => {
      await expect(client.getAll!(headerEmpty)).rejects.toHaveProperty(
        "response.status",
  ...
  describe("[header] Missing Username", () => {
    it("should return HTTP 401 - UNAUTHORIZED", async () => {
      await expect(client.getAll!(headerWithoutUser)).rejects.toHaveProperty(
        "response.status",
        HttpStatus.UNAUTHORIZED,
      );
    });
  ...
  describe("[header] Valid header without permission", () => {
    it("should return HTTP 403 - FORBIDDEN", async () => {
      await expect(client.getAll!(headerForbidden)).rejects.toHaveProperty(
        "response.status",
