...
type MethodGetById = (requestHeader: Record<string, string>, id: string) => Response
type MethodCreate = (requestHeader: Record<string, string>, data: any) => Response
type MethodUpdateById = (requestHeader: Record<string, string>, id: string, data: any) => Response
type MethodDeleteById = (requestHeader: Record<string, string>, id: string) => Response
...
export abstract class AbstractClient {
  ...
  constructor(apiPrefix: string, config: ClientMethodConfig = FULL_CLIENT_CONFIG) {
    this.configureMethods(apiPrefix, config)
  }

  private configureMethods(apiPrefix: string, config: ClientMethodConfig): void {
  ...
  private createCreateMethod(baseUrl: string): MethodCreate {
    return async (requestHeader: Record<string, string>, data: any) => {
      return await this.makeRequestWithRetry('post', requestHeader, baseUrl, data)
    }
  }

  private createUpdateByIdMethod(baseUrl: string): MethodUpdateById {
    return async (requestHeader: Record<string, string>, id: string, data: any) => {
      const url = `${baseUrl}/${id}`
  ...
  private async makeRequestWithRetry(
    ...
    for (let retries = 0; retries < MAX_RETRIES; retries += 1) {
      ...
      try {
        this.logger.debug(`[attempt #${retries}] making request at: ${url}`
          + `, method: ${method}`
        ...
        this.logger.debug(`[attempt #${retries}] received response from: ${url}`
          + `, method: ${method}`
